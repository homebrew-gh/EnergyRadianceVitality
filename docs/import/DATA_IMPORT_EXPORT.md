...
- **Relay uploads (all kind-30078 fitness data + import):** Encrypted kind **30078** publishes go through a durable **`RelayPublishOutbox`** (DataStore): interactive sync and import enqueue plaintext JSON by **`d` tag** (at most one pending row per tag; newer payloads replace older queued rows for the same tag). **`kickDrain`** sends sequentially with **~150 ms** spacing and **exponential backoff** on failure. **`MainActivity`** also drains the outbox when relays/signer are available (e.g. after relay URL changes). **Local DataStore remains canonical**; failed sends stay queued and retry.
- **Cardio (implemented import):** **`docs/import/cardio_training_import_ai_guide.md`** (JSON, **built-in activity enum table**, Strava → ERV hints), **`docs/import/cardio_training_import_csv_guide.md`**, and **`docs/import/cardio_training_nostr_events_reference.md`** (*optional*—`d` tags / kind **30078** for relay sync, **not** required to author imports). JSON (primary) and ERV cardio CSV ship under **Import And Export** with the same preview/merge pattern as weight. Sessions use `source`: **`IMPORTED`**.
- **Programs (implemented import):** **`docs/import/programs_import_ai_guide.md`** — JSON envelope or single program for **weekly programs** (day blocks: weight, cardio, stretch, heat/cold, rest, custom). Ships in-app under **Import And Export** with **preview then merge** (same `id` replaces an existing program; optional **`activeProgramId`**). Data is **local-only** in the current build (no relay publish for programs).
