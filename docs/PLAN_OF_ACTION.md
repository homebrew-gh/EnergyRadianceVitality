...
| **Body weight (optional)** | `erv/bodyweight`      | Diary: list of date → weight (kg); one replaceable event (may shard by year if large); used for Cardio calorie estimation; see §2.4 |
...
- **Content (encrypted JSON)**: e.g. `{ "entries": [ { "date": "YYYY-MM-DD", "weightKg": 75.5 }, ... ] }` — list of date + weight (kg), sorted by date. **Payload growth:** daily logging for 2+ years means 730+ entries (~36 KB JSON); within most relay limits but monitor size over time. Optionally cap, prune old entries, or shard by year (e.g. `erv/bodyweight/2026`, `erv/bodyweight/2027`) if the single event grows too large for reliable sync.
- **UI**: User can **log weight** (e.g. daily or when they choose); view **history** (list by date) and optionally a **simple trend** (e.g. line chart over time). Access from Settings, a “Profile” area, or a small “Body weight” entry point; need not be a full dashboard tile unless desired.
- **Use elsewhere**: **Cardio calorie estimation** (MET × weight × time) uses the **latest diary entry** or the entry for the session date when available; if the user does not use the diary, fall back to a single weight in Settings or skip estimate. **Rucking** (Pandolf) also uses body mass from this diary or Settings.
...
**5. Navigation** — Dashboard (main), **Goals** (tab for editing goals), **Settings** (see §7.5 for full inventory: relay, keys/login, theme, GPS, body weight fallback, max HR/age, temperature unit, etc.). Category content is reached from dashboard tiles or Goals section rows.
...
| **Body weight (fallback)** | **Single weight (kg)** used when the user does not use the body weight diary. Used for Cardio calorie estimation (MET × weight × time) and rucking (Pandolf). If unset and no diary entry exists, prompt or skip estimate. Optionally: entry point to body weight diary or “use latest diary value.” | Cardio (§2.4) calorie estimation; body weight diary (§2.4). |
...
4. **Local data and first categories: Light therapy + Supplements**  
...
5. **Dashboard, theme, and date navigation**  
   ...
   - Navigation into implemented categories (Light therapy, Supplements); others show Coming soon or similar.
...
7. **Cardio and Body weight diary**  
   - **Body weight diary** (§2.4) first: `erv/bodyweight`, entries by date (weight kg); UI to log and view history/trend. Build **before or alongside** Cardio because calorie estimation (MET x weight x time) depends on body weight. If the user does not use the diary, fall back to a single weight in Settings or skip estimate.  
...
|-----|------------------|----------------|
| **Body weight for calories** | Cardio MET/Pandolf formulas need weight (kg). | Use optional **body weight diary** (§2.4) — latest or session-date entry; fallback to single weight in Settings if user doesn’t use diary. Prompt or skip estimate if neither is set. |
