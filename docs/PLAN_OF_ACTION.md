...
**Companion website and web-of-trust sharing** — This plan does **not** lay out a separate web product in detail. The product direction for trust-filtered routine discovery, forking, and a richer web surface lives in [PROTOCOL_GRAPH.md](PROTOCOL_GRAPH.md). Tradeoffs for fitness promotion and community are in [COMPANION_WEB_COMMUNITY_PITFALLS_BENEFITS.md](COMPANION_WEB_COMMUNITY_PITFALLS_BENEFITS.md). [DATA_IMPORT_EXPORT.md](DATA_IMPORT_EXPORT.md) ties import/export contracts to that graph at a technical level.
