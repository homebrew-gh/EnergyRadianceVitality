...
- **Programs (implemented import):** **`docs/programs_import_ai_guide.md`** — JSON envelope or single program for **weekly programs** (day blocks: weight, cardio, stretch, heat/cold, rest, custom). Ships in-app under **Import And Export** with **preview then merge** (same `id` replaces an existing program; optional **`activeProgramId`**). Data is **local-only** in the current build (no relay publish for programs).
