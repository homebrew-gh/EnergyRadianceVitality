...
| **E** | Done | Activity-scoped `WeightLiveWorkoutViewModel`; full-screen **Live workout** (elapsed timer, add/reorder/remove exercises, per-exercise set editor); **Recent workouts** on each lift (`WeightExerciseRecentWorkoutsDialog`) → last 5 logged sessions from `historyForExercise` for reference; same control on **manual log editor**; **Start workout** (blank) + **Play** on routines; **Finish** → `addWorkout` for **`LocalDate.now()`** only if ≥1 logged set; **Cancel** with confirm when draft non-empty; `WeightSync.publishDayLog`; blocks second start with snackbar |
...
| **J** | Done | **Docs**: [PLAN_OF_ACTION.md](../PLAN_OF_ACTION.md) §2.3 identifier note for weight daily logs; [PROTOCOL_GRAPH.md](vision/PROTOCOL_GRAPH.md) §15 Nostr d-tag reference for shipped silos. |
...
**How to verify stage E:** Start blank workout → add exercise → **Recent workouts** → see prior sessions for that lift (if any) → dismiss → log sets (reps required) → Finish → today’s log contains session (check relay or re-open app). Start from routine → list prefilled. Try second start → snackbar. Cancel with exercises → confirm discard.
...
Avoid implementing from stale [PLAN_OF_ACTION.md](../PLAN_OF_ACTION.md) / [PROTOCOL_GRAPH.md](vision/PROTOCOL_GRAPH.md) while stages B–I are in progress:
