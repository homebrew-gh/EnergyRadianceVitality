...
private fun guidedSessionTotalMinutes(nStretches: Int, holdSeconds: Int): Int {
    ...
    return max(1, (holdTotal + transitions + 59) / 60)
}
...
private fun StretchGuidedSessionOverlay(
    ...
    var phaseHold by remember { mutableStateOf(true) }
    var index by remember { mutableIntStateOf(0) }
    var secondsLeft by remember { mutableIntStateOf(holdSeconds) }
    ...
    LaunchedEffect(stretchEntries, holdSeconds) {
        var i = 0
                        ...
                        LazyColumn(
                            ...
                            items(catalog, key = { it.id }) { entry ->
                                TextButton(
                                    ...
                                    Text(entry.name, modifier = Modifier.fillMaxWidth())
                                }
                            }
                        }
                    }
