...
# Build rootfs outside the repo by default — a workspace rootfs freezes Cursor's indexer.
if [[ -n "${ERV_ROOTFS_DIR:-}" ]]; then
  ROOTFS="$ERV_ROOTFS_DIR"
  ROOTFS_IS_TEMP=0
else
  ROOTFS="$(mktemp -d -t erv-web-rootfs.XXXXXX)"
  ROOTFS_IS_TEMP=1
fi
IMAGE_TAG="${1:-start9/erv-web/main:0.4.0}"
DOCKER="${DOCKER:-docker}"

cleanup_on_exit() {
  if [[ "${ROOTFS_IS_TEMP:-0}" == "1" && -n "${ROOTFS:-}" ]]; then
...
install_app_files() {
  local use_sudo="${1:-0}"
  if [[ "$use_sudo" == "1" ]]; then
    sudo install -D -m 0755 "$PACK/erv-web" "$ROOTFS/usr/local/bin/erv-web"
    ...
    install -D -m 0755 "$PACK/erv-web" "$ROOTFS/usr/local/bin/erv-web"
...
if [[ -f "$PACK/Dockerfile" && -f "$PACK/erv-web" && -d "$PACK/dist" ]]; then
  ...
  run_docker import \
    --change 'ENV ERV_DATA_DIR=/data ERV_STATIC_DIR=/srv/dist ERV_PORT=3000 ERV_BIND=0.0.0.0 ERV_LOG=info ERV_INSECURE_RELAY_TLS=1' \
    --change 'EXPOSE 3000' \
    --change 'VOLUME ["/data"]' \
    --change 'HEALTHCHECK --interval=30s --timeout=5s --start-period=15s --retries=3 CMD curl -fsS http://127.0.0.1:3000/api/health || exit 1' \
    --change 'ENTRYPOINT ["/usr/local/bin/erv-web"]' \
    - "$IMAGE_TAG" < "$TAR"

  rm -f "$TAR"
  if [[ "${ROOTFS_IS_TEMP:-0}" == "1" ]]; then
    cleanup_rootfs "$ROOTFS"
    trap - EXIT
  fi
